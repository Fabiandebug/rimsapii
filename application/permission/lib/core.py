...
extremely simple and incomplete example:
    ...
    from application.permission.lib.core import configure_permissions, Allow, Deny
    from pydantic import BaseModel
    app = FastAPI()
    oauth2_scheme = OAuth2PasswordBeare2(tokenUrl="/token")
...
from ...session.models import Principals, User as UserInDB 
# constants
...
ALLOW_ALL = (Allow, Everyone, All)  # acl shorthand, allows everything
...
# permission_exception = CustomException(
#     status_code=403,
#     detail={"message": "Permission Denied."}
# )
permission_exception = CustomException(error="Insufficient permission", status=HTTP_403_FORBIDDEN)


def configure_permissions(
    active_principals_func: Any,
    permission_exception: CustomException = permission_exception,
):
...
def has_permission(
    ...
    print('requested permission: ', requested_permission)
...
async def check_permission(requested_permission:str, resource: Any) -> bool:
    user_principals = await get_active_principals()
    if has_permission(user_principals, requested_permission, resource) == False:
        raise permission_exception
    else:
        return True 
...
def _normalize_acl(resource: Any):
...
async def get_active_principals():
    ...
    db = get_db()
    user_id = context.data.get("user").get("id")
    user = UserInDB.get_user_by_id(db, user_id)
